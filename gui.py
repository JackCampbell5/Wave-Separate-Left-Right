...
def run_wave_seperate():
    wave_seperate_method(outputVar[0], outputVar[1], outputVar[2])


def add_output(num, what):
    outputVar[num] = what
    print(what)
...
l_title.grid(row=0, column=0, columnspan=4, padx=defult_padx, pady=(20, 5))
# Into Label and directors
l_title_l = Label(window, text="Insert Director text here", font=('', 12))
l_title_l.grid(row=1, column=0, columnspan=4, padx=defult_padx, pady=(5, 20), sticky='W')

# Selection button 0 label 0
labels_button_zero = Label(window, text="Select target directory", font=('', 18))
labels_button_zero.grid(row=2, column=0, columnspan=2, padx=defult_padx, pady=(20, 1), sticky='W')
# Selection button 0
...
# Selection label 0
labels[0] = Label(window, text=".", font=('', 14))
...
labels_button_one.grid(row=4, column=0, columnspan=2, padx=defult_padx, pady=(20, 1), sticky='W')
# Label of checkbox
labels_new = Label(window, text="Sub Folder?", font=('', 14))
labels_new.grid(row=5, column=0, columnspan=2, padx=defult_padx, pady=(20, 1), sticky='W')
# # Checkbox creation
# checkbox_output = IntVar()
# checkbox = Checkbutton(window, variable=checkbox_output, command=lambda: add_output(3))
# checkbox.grid(row=5, column=1, columnspan=2, padx=defult_padx, pady=(20, 1), sticky='W')
# Selection button 1
...
# Selection label 1
labels[1] = Label(window, text=".", font=('', 14))
...
labels_button_two.grid(row=7, column=0, columnspan=2, padx=defult_padx, pady=(20, 1), sticky='W')
# Selection button 2
...
# Run Button
buttons[3] = Button(window, text='Run Program', font=('', 12), command=lambda: run_wave_seperate(),
                    state="disabled")
